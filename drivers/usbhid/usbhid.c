...
void keyboard_init(void) {
    ...
    // Note: This should be called after all basic system initialization
...
#if CFG_TUH_RPI_PIO_USB
    // Configure PIO-USB for USB-A connector on GPIO 28/29
    pio_usb_configuration_t pio_cfg = PIO_USB_DEFAULT_CONFIG;
    pio_cfg.pin_dp = PIO_USB_DP_PIN_DEFAULT;  // GPIO 28 (D-=29)
    
    // Pass PIO configuration to the host stack (port 1 = USB-A)
    // Note: tuh_configure() must be called before tuh_init()
    tuh_configure(1, TUH_CFGID_RPI_PIO_USB_CONFIGURATION, &pio_cfg);
#endif
    
    // Initialize host stack: port 0=micro USB, port 1=USB-A (PIO-USB)
    bool init_result = tuh_init(1);
    if (!init_result) {
